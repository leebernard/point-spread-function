...
def elliptical_Moffat(indata, flux, x0, y0, beta, a, b):
    """Model of PSF using a single Moffat distribution, with elliptical parameters.

    This function does not account for angle of eccentricity, forcing the elliptical
    to be along rows and columns of the CCD grid
    ...
    b: float
        width parameter in the y directiony
    offset: float
...
def flat_elliptical_Moffat(indata, flux, x0, y0, beta, a, b):
    """Model of PSF using a single Moffat distribution, with elliptical parameters.
...
for n, filename in enumerate(filename_list):
    ...
    for m, aperture in enumerate(aperture_list):
        ...
        a_bound = [0.1, np.inf]
        b_bound = [0.1, np.inf]
        theta_bound = 0, np.pi/2] REMOVED
        offset_bound = [-np.inf, np.inf] REMOVED
        """
        # format the bounds
        lower_bounds = [0, 0, 0, 1, 0.1, 0.1]
        upper_bounds = [np.inf, aperture.shape[1], aperture.shape[0], 20, np.inf, np.inf]
        ...
        guess = [flux_guess, x_guess, y_guess, beta_guess, a_guess, b_guess]
        ...
        try:
            ...
            result = elliptical_Moffat(m_input, m_flux, m_x0, m_y0, m_beta, m_a, m_b)
            ...
            expected = flat_elliptical_Moffat(m_input, m_flux, m_x0, m_y0, m_beta, m_a, m_b)
